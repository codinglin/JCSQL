package cn.edu.gzhu.backend.parser;
...
public class Parser {
    public static Object parse(byte[] statement) throws Exception {
        ...
        tokenizer.pop();
        Object state = null;
        Exception stateErr = null;
        try {
            switch (token) {
                case "begin":
                    state = parseBegin(tokenizer);
                    break;
                case "commit":
                    state = parseCommit(tokenizer);
                    break;
                case "abort":
                    state = parseAbort(tokenizer);
                    break;
                case "create":
                    state = parseCreate(tokenizer);
                    break;
                case "drop":
                    state = parseDrop(tokenizer);
                    break;
                case "select":
                    state = parseSelect(tokenizer);
                    break;
                case "insert":
                    state = parseInsert(tokenizer);
                    break;
                case "delete":
                    state = parseDelete(tokenizer);
                    break;
                case "update":
                    state = parseUpdate(tokenizer);
                    break;
                case "show":
                    state = parseShow(tokenizer);
                    break;
                default:
        ...
        try {
            ...
            if(!"".equals(next)) {
                byte[] errStat = tokenizer.errState();
                stateErr = new RuntimeException("Invalid statement: " + new String(errStat));
            }
        } catch(Exception e) {
            e.printStackTrace();
            byte[] errStat = tokenizer.errState();
    ...
    private static Where parseWhere(Tokenizer tokenizer) throws Exception {
        ...
        String logicOperation = tokenizer.peek();
        if("".equals(logicOperation)) {
            where.logicOp = logicOperation;
            return where;
        }
        if(!isLogicOperation(logicOperation)) {
            throw Error.InvalidCommandException;
        }
        where.logicOp = logicOperation;
        tokenizer.pop();
    ...
    private static boolean isLogicOperation(String operation) {
        return ("and".equals(operation) || "or".equals(operation));
    }
    ...
    private static SingleExpression parseSingleExp(Tokenizer tokenizer) throws Exception {
        ...
        String operation = tokenizer.peek();
        if(!isCompareOperation(operation)) {
            throw Error.InvalidCommandException;
        }
        exp.compareOp = operation;
        tokenizer.pop();
    ...
    private static boolean isCompareOperation(String operation) {
        return ("=".equals(operation) || ">".equals(operation) || "<".equals(operation));
    }
    ...
    private static boolean isType(String filedType) {
        return ("int32".equals(filedType) || "int64".equals(filedType) || "string".equals(filedType));
    }
    ...
    private static Begin parseBegin(Tokenizer tokenizer) throws Exception {
        ...
        String temp1 = tokenizer.peek();
        if("read".equals(temp1)) {
            tokenizer.pop();
            String temp2 = tokenizer.peek();
            if("committed".equals(temp2)) {
                tokenizer.pop();
                if(!"".equals(tokenizer.peek())) {
                    throw Error.InvalidCommandException;
                }
        ...
        } else if ("repeatable".equals(temp1)) {
            tokenizer.pop();
            String temp2 = tokenizer.peek();
            if("read".equals(temp2)) {

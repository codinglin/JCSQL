...
public class Tokenizer {
    ...
    public byte[] errState(){
        byte[] res = new byte[statement.length + 3];
        System.arraycopy(statement, 0, res, 0, position);
        System.arraycopy("<< ".getBytes(), 0, res, position, 3);
        System.arraycopy(statement, position, res, position + 3, statement.length - position);
        return res;
    }

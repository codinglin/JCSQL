...
import cn.edu.gzhu.backend.tbm.Field.ParseValueRes;
...
public class Table {
    ...
    public static Table loadTable(TableManager tableManager, long uid) {
        byte[] raw = null;
        try {
            raw = ((TableManagerImpl) tableManager).vm.read(TransactionManagerImpl.SUPER_XID, uid);
        } catch (Exception e) {
            Panic.panic(e);
        }
        assert raw != null;
    ...
    private Table persistSelf(long xid) throws Exception {
        ...
        uid = ((TableManagerImpl) tableManager).vm.insert(xid, Bytes.concat(nameRaw, nextRaw, fieldRaw));
        return this;
    }
    ...
    public int delete(long xid, Delete delete) throws Exception {
        ...
        int count = 0;
        for (Long uid : uids) {
            if(((TableManagerImpl)tableManager).vm.delete(xid, uid)) {
                count ++;
            }
        }
        return count;
    ...
    public int update(long xid, Update update) throws Exception {
        ...
        int count = 0;
        for (Long uid : uids) {
            byte[] raw = ((TableManagerImpl)tableManager).vm.read(xid, uid);
            if(raw == null) continue;

            ((TableManagerImpl)tableManager).vm.delete(xid, uid);

            Map<String, Object> entry = parseEntry(raw);
            entry.put(fd.fieldName, value);
            raw = entry2Raw(entry);
            long uuid = ((TableManagerImpl)tableManager).vm.insert(xid, raw);
    ...
    public void insert(long xid, Insert insert) throws Exception {
        ...
        long uid = ((TableManagerImpl)tableManager).vm.insert(xid, raw);
        for (Field field : fields) {
            if(field.isIndexed()) {
